...
@push('styles')
    ...
    <style>
        ...
        .dataTables_wrapper .dataTables_length select {
            ...
            margin-right: 0.5rem;
            padding-right: 2rem;
            /* Ruang untuk panah dropdown bawaan browser */
        }
        ...
        /* Memberikan jarak antara label 'Cari:' dan form inputnya */
...
@section('content')
    <header class="flex flex-row justify-between items-center mb-10">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl md:text-3xl font-extrabold text-slate-800">Master Data Kategori</h2>
                <p class="text-slate-400 text-sm md:text-base mt-1">Kelola daftar kategori tema undangan Anda di sini.</p>
            </div>
        </div>
    ...
    <section>
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            <div class="overflow-x-auto">
                <table id="table-category" class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            ...
                            <th class="p-6 font-bold">Nama Kategori</th>
                            <th class="p-6 font-bold">Slug URL</th>
                            <th class="p-6 font-bold">Total Template</th>
                    ...
                    <tbody class="text-sm">
                        @foreach ($categories as $category)
                            <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition duration-200">
                                ...
                                <td class="p-6 font-bold text-slate-800">{{ $category->name }}</td>
                                <td class="p-6">
                                    <span
                                        class="px-3 py-1 bg-slate-100 text-slate-500 text-xs font-bold rounded-lg">{{ $category->slug }}</span>
                                </td>
                                <td class="p-6 font-semibold text-slate-600">{{ $category->templates->count() }} Tema</td>
                                <td class="p-6 flex justify-center gap-2">
                                    ...
                                    <form id="delete-form-{{ $category->id }}"
                                        ...
                                        @method('DELETE')
                                        <button type="button"
                                            onclick="openCustomModal('delete', 'delete-form-{{ $category->id }}', 'PERINGATAN: Menghapus kategori akan menghapus SEMUA Tema yang menggunakan kategori ini. Yakin lanjutkan?')"
...
@push('scripts')
    ...
    <script>
        ...
        function openCustomModal(type, formId, message) {
            ...
            // Trigger animasi scale dan sedikit delay agar mata mengikuti gerakannya
            setTimeout(() => {
