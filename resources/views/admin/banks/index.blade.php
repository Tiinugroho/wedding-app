...
@section('title', 'Daftar Bank & E-Wallet')
...
@section('content')
    <header class="flex flex-row justify-between items-center mb-10">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl md:text-3xl font-extrabold text-slate-800">Master Data Bank</h2>
                <p class="text-slate-400 text-sm md:text-base mt-1">Kelola daftar bank & e-wallet untuk hadiah digital.</p>
            </div>
        </div>
        <a href="{{ route('admin.banks.create') }}" class="hidden md:flex items-center gap-2 bg-gradient-to-r from-rRed to-rOrange text-white px-6 py-3 rounded-full font-bold text-sm shadow-lg shadow-rRed/20 hover:scale-105 transition-transform">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
            Tambah Bank
        </a>
    </header>

    <a href="{{ route('admin.banks.create') }}" class="md:hidden flex items-center justify-center gap-2 bg-gradient-to-r from-rRed to-rOrange text-white px-6 py-3 rounded-full font-bold text-sm shadow-lg shadow-rRed/20 hover:scale-105 transition-transform mb-6">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
        Tambah Bank
    </a>
    ...
    <section>
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            <div class="overflow-x-auto">
                <table id="table-bank" class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-slate-400 text-xs uppercase tracking-widest border-b border-slate-100 bg-slate-50/50">
                            ...
                            <th class="p-6 font-bold">Logo</th>
                            <th class="p-6 font-bold">Nama Bank / E-Wallet</th>
                    ...
                    <tbody class="text-sm">
                        @foreach($banks as $bank)
                            <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition duration-200">
                                ...
                                <td class="p-6">
                                    <div class="w-16 h-10 flex items-center justify-center bg-white border border-slate-100 rounded-lg p-1 shadow-sm">
                                ...
                                <td class="p-6 font-extrabold text-slate-800">{{ $bank->name }}</td>
                                <td class="p-6 text-center">
                                    @if($bank->is_active)
                                        <span class="px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-bold uppercase rounded-lg tracking-wider border border-emerald-100">Aktif</span>
                                    @else
                                        <span class="px-3 py-1 bg-slate-50 text-slate-500 text-[10px] font-bold uppercase rounded-lg tracking-wider border border-slate-200">Nonaktif</span>
                                    @endif
                                </td>
                                <td class="p-6 flex justify-center gap-2 items-center h-full mt-1">
                                    <a href="{{ route('admin.banks.edit', $bank->id) }}" class="p-2 bg-amber-50 text-amber-500 hover:bg-amber-500 hover:text-white rounded-xl transition shadow-sm" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <form id="delete-form-{{ $bank->id }}" action="{{ route('admin.banks.destroy', $bank->id) }}" method="POST" class="d-inline m-0">
                                        @csrf @method('DELETE')
                                        <button type="button" onclick="openCustomModal('delete', 'delete-form-{{ $bank->id }}', 'Apakah Anda yakin ingin menghapus bank ini?')" class="p-2 bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition shadow-sm" title="Hapus">
    ...
    <script>
        $(document).ready(function() {
            $('#table-bank').DataTable({
                ...
                columnDefs: [{ orderable: false, targets: [1, 3, 4] }] // Mematikan sorting untuk Logo, Status, Aksi
            });
        });
