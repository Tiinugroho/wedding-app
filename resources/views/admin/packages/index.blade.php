...
@section('content')
    <header class="flex flex-row justify-between items-center mb-10">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl md:text-3xl font-extrabold text-slate-800">Master Data Paket</h2>
                <p class="text-slate-400 text-sm md:text-base mt-1">Kelola daftar paket harga dan fitur undangan.</p>
            </div>
        </div>
        <a href="{{ route('admin.packages.create') }}" class="hidden md:flex items-center gap-2 bg-gradient-to-r from-rRed to-rOrange text-white px-6 py-3 rounded-full font-bold text-sm shadow-lg shadow-rRed/20 hover:scale-105 transition-transform">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
            Tambah Paket
        </a>
    </header>

    <a href="{{ route('admin.packages.create') }}" class="md:hidden flex items-center justify-center gap-2 bg-gradient-to-r from-rRed to-rOrange text-white px-6 py-3 rounded-full font-bold text-sm shadow-lg shadow-rRed/20 hover:scale-105 transition-transform mb-6">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
        Tambah Paket
    </a>
    ...
    <section>
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            <div class="overflow-x-auto">
                <table id="table-paket" class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-slate-400 text-xs uppercase tracking-widest border-b border-slate-100 bg-slate-50/50">
                            ...
                            <th class="p-6 font-bold">Nama Paket</th>
                            <th class="p-6 font-bold">Harga Jual</th>
                            <th class="p-6 font-bold">Harga Coret</th>
                    ...
                    <tbody class="text-sm">
                        @foreach ($packages as $package)
                            <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition duration-200">
                                ...
                                <td class="p-6 font-extrabold text-slate-800">{{ $package->name }}</td>
                                <td class="p-6 font-bold text-rOrange">Rp {{ number_format($package->price, 0, ',', '.') }}</td>
                                <td class="p-6 text-slate-400">
                                    @if($package->original_price)
                                        <span class="line-through">Rp {{ number_format($package->original_price, 0, ',', '.') }}</span>
                                    @else
                                        -
                                    @endif
                                </td>
                                <td class="p-6 text-center">
                                    @if($package->is_active)
                                        <span class="px-3 py-1 bg-green-50 text-green-500 text-[10px] font-bold uppercase rounded-lg tracking-wider">Aktif</span>
                                    @else
                                        <span class="px-3 py-1 bg-slate-100 text-slate-500 text-[10px] font-bold uppercase rounded-lg tracking-wider">Nonaktif</span>
                                    @endif
                                </td>
                                <td class="p-6 flex justify-center gap-2">
                                    <a href="{{ route('admin.packages.edit', $package->id) }}" class="p-2 bg-amber-50 text-amber-500 hover:bg-amber-500 hover:text-white rounded-xl transition shadow-sm" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <form id="delete-form-{{ $package->id }}" action="{{ route('admin.packages.destroy', $package->id) }}" method="POST" class="d-inline m-0">
                                        @csrf @method('DELETE')
                                        <button type="button" onclick="openCustomModal('delete', 'delete-form-{{ $package->id }}', 'Apakah Anda yakin ingin menghapus paket ini? Template yang terhubung ke paket ini mungkin terdampak.')" class="p-2 bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition shadow-sm" title="Hapus">
