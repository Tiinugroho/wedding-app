<div id="sidebar-backdrop" class="fixed inset-0 z-40 hidden bg-slate-900/50 backdrop-blur-sm lg:hidden transition-opacity duration-300" onclick="toggleSidebar()"></div>

<aside id="admin-sidebar" class="fixed inset-y-0 left-0 z-50 flex flex-col w-72 glass-sidebar h-screen p-8 transform -translate-x-full lg:translate-x-0 lg:static lg:sticky lg:top-0 transition-transform duration-300 ease-in-out">
    
    <button type="button" onclick="toggleSidebar()" class="absolute top-6 right-6 lg:hidden bg-white p-2 rounded-xl border border-slate-100 shadow-sm text-slate-400 hover:text-rRed hover:border-rRed focus:outline-none focus:ring-2 focus:ring-rOrange transition">
