...
<head>
    ...
    <style>
        ...
        body { overflow-y: hidden; background-color: #0F0F0F; color: #FFFFFF; -webkit-font-smoothing: antialiased; }
        .floating-nav { background: rgba(15, 15, 15, 0.75); backdrop-filter: blur(15px); -webkit-backdrop-filter: blur(15px); border: 1px solid rgba(197, 160, 101, 0.25); box-shadow: 0 15px 35px rgba(0, 0, 0, 0.5); }
        .landing-mist { background: linear-gradient(to bottom, rgba(0, 0, 0, 0.2) 0%, rgba(15, 15, 15, 0.95) 100%); }
        .text-protected { text-shadow: 0 2px 10px rgba(0, 0, 0, 0.5); }
        @keyframes fade-in-up { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
        .animate-fade-in-up { animation: fade-in-up 1.2s cubic-bezier(0.23, 1, 0.32, 1) forwards; }
        .gold-glow { filter: drop-shadow(0 0 5px rgba(197, 160, 101, 0.3)); }
        .gold-gradient-text { background: linear-gradient(to right, #BF953F, #FCF6BA, #B38728, #FBF5B7, #AA771C); -webkit-background-clip: text; -webkit-text-fill-color: transparent; display: inline-block; }
        .glass-card { background: rgba(255, 255, 255, 0.03); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border: 1px solid rgba(255, 255, 255, 0.1); }
        .shimmer { position: relative; overflow: hidden; }
        .shimmer::after { content: ''; position: absolute; top: -50%; left: -50%; width: 200%; height: 200%; background: linear-gradient(45deg, transparent, rgba(255,255,255,0.1), transparent); transform: rotate(45deg); animation: shimmer-effect 4s infinite; }
        @keyframes shimmer-effect { 0% { left: -100%; } 100% { left: 100%; } }
        @keyframes slide-up-soft { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
        .animate-slide-up { animation: slide-up-soft 0.5s ease-out forwards; }
        .input-luxury { background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.1); color: white; transition: all 0.3s ease; }
        .input-luxury:focus { border-color: #C5A065; background: rgba(255, 255, 255, 0.07); box-shadow: 0 0 15px rgba(197, 160, 101, 0.1); }
        ...
        .glass-stat { background: linear-gradient(135deg, rgba(255, 255, 255, 0.05) 0%, rgba(255, 255, 255, 0.02) 100%); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.1); }
    </style>
...
<body class="bg-brand-dark text-white font-sans antialiased relative selection:bg-brand-gold selection:text-brand-dark">
    ...
    <div id="cover-page" class="fixed inset-0 z-50 flex items-center justify-center w-screen h-screen bg-brand-dark overflow-hidden transition-transform duration-1000 ease-in-out">
        <div class="absolute inset-0 z-0">
            <div class="absolute inset-0 opacity-30 animate-slow-zoom bg-[url('{{ !empty($content['groom_photo']) ? asset('storage/'.$content['groom_photo']) : 'https://images.unsplash.com/photo-1511795409834-ef04bbd61622?q=80&w=2069&auto=format&fit=crop' }}')] bg-cover bg-center"></div>
        ...
        <div class="relative z-20 w-full max-w-[90%] md:max-w-2xl lg:max-w-3xl px-6 py-12 flex flex-col items-center justify-center min-h-[600px]">
            ...
            <div class="text-center mb-8">
                <h1 class="text-6xl md:text-8xl lg:text-9xl font-serif text-white tracking-tighter leading-[0.85] flex flex-col md:flex-row items-center justify-center md:gap-8">
                    <span>{{ $isGroomFirst ? $pria : $wanita }}</span>
                    <span class="italic font-light text-brand-gold text-4xl md:text-7xl lg:text-8xl my-2 md:my-0">&</span>
                    <span>{{ $isGroomFirst ? $wanita : $pria }}</span>
                </h1>
            </div>
    ...
    <main id="main-content" class="min-h-screen pb-28 opacity-0 transition-opacity duration-1000 bg-brand-dark">
        ...
        <section id="home" class="min-h-screen flex flex-col items-center justify-center text-center p-6 md:p-12 relative overflow-hidden">
            <div class="absolute inset-0 z-0">
                <div class="absolute inset-0 bg-[url('{{ !empty($content['bride_photo']) ? asset('storage/'.$content['bride_photo']) : 'https://images.unsplash.com/photo-1511285560929-80b456fea0bc?q=80&w=1000&auto=format&fit=crop' }}')] bg-cover bg-center bg-fixed opacity-20"></div>
                    ...
                    <div class="flex flex-row justify-center items-center gap-4 md:gap-12">
                        <div class="flex flex-col items-center">
                            <div class="relative flex items-center justify-center w-20 h-20 md:w-32 md:h-32">
                                <svg class="absolute inset-0 w-full h-full -rotate-90 opacity-20"><circle cx="50%" cy="50%" r="48%" stroke="currentColor" stroke-width="2" fill="none" class="text-brand-gold"></circle></svg>
            ...
            <div class="max-w-6xl mx-auto relative z-10">
                ...
                @php
                    ...
                    <div class="flex flex-col md:flex-row items-center gap-8 md:gap-12 lg:pr-12 group">
                        <div class="relative w-64 h-80 md:w-72 md:h-[28rem] order-1 md:order-2 flex-shrink-0">
                            ...
                            <div class="absolute inset-0 bg-brand-accent overflow-hidden rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-10">
                                <img src="'.(!empty($content['groom_photo']) ? asset('storage/'.$content['groom_photo']) : 'https://images.soco.id/230-58.jpg.jpeg').'" class="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" alt="Groom">
                            </div>
                        </div>
                        <div class="text-center md:text-right order-2 md:order-1 flex-1">
                            <p class="text-brand-gold font-serif italic text-2xl mb-2">The Groom</p>
                            <h3 class="text-4xl md:text-5xl font-serif text-white mb-6 tracking-wide leading-tight">'.($content['groom_name'] ?? 'Romeo Montague').'</h3>
                            <div class="space-y-3">
                                <p class="uppercase text-[10px] tracking-[0.3em] text-brand-gold font-bold">Putra Tercinta dari</p>
                                <p class="text-white text-lg font-medium">'.($content['groom_father'] ?? 'Bapak Fulan').'</p>
                                <p class="text-white text-lg font-medium">& '.($content['groom_mother'] ?? 'Ibu Fulanah').'</p>
                            </div>
                            '.(!empty($content['groom_ig']) ? '<a href="https://instagram.com/'.$content['groom_ig'].'" target="_blank" class="inline-flex items-center gap-2 mt-6 text-brand-gold/60 hover:text-brand-gold transition text-xs font-semibold tracking-widest uppercase"><i class="fa-brands fa-instagram"></i> '.$content['groom_ig'].'</a>' : '').'
                        </div>
                    </div>';
                    ...
                    <div class="flex flex-col md:flex-row items-center gap-8 md:gap-12 lg:pl-12 group mt-0 lg:mt-40">
                        <div class="relative w-64 h-80 md:w-72 md:h-[28rem] flex-shrink-0">
                            <div class="absolute inset-0 border-2 border-brand-gold -translate-x-4 translate-y-4 rounded-2xl z-0"></div>
                            <div class="absolute inset-0 bg-brand-accent overflow-hidden rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-10">
                                <img src="'.(!empty($content['bride_photo']) ? asset('storage/'.$content['bride_photo']) : 'https://images.pexels.com/photos/157757/wedding-dresses-fashion-character-bride-157757.jpeg').'" class="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" alt="Bride">
                            </div>
                        </div>
                        <div class="text-center md:text-left flex-1">
                            <p class="text-brand-gold font-serif italic text-2xl mb-2">The Bride</p>
                            <h3 class="text-4xl md:text-5xl font-serif text-white mb-6 tracking-wide leading-tight">'.($content['bride_name'] ?? 'Juliet Capulet').'</h3>
                            <div class="space-y-3">
                                <p class="uppercase text-[10px] tracking-[0.3em] text-brand-gold font-bold">Putri Tercinta dari</p>
                                <p class="text-white text-lg font-medium">'.($content['bride_father'] ?? 'Bapak Fulan').'</p>
                                <p class="text-white text-lg font-medium">& '.($content['bride_mother'] ?? 'Ibu Fulanah').'</p>
                            </div>
                            '.(!empty($content['bride_ig']) ? '<a href="https://instagram.com/'.$content['bride_ig'].'" target="_blank" class="inline-flex items-center gap-2 mt-6 text-brand-gold/60 hover:text-brand-gold transition text-xs font-semibold tracking-widest uppercase"><i class="fa-brands fa-instagram"></i> '.$content['bride_ig'].'</a>' : '').'
                        </div>
                    </div>';
                @endphp
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-20 lg:gap-12 items-center">
                    @if($isGroomFirst)
                        ...
                        {!! $brideBlock !!}
                    @else
                        {{-- Hapus mt-40 dari bride jika di atas, berikan ke groom --}}
                        {!! str_replace('lg:mt-40', '', $brideBlock) !!}
                        {!! str_replace('group', 'group lg:mt-40', $groomBlock) !!}
                    @endif
                </div>
                ...
                @if(!empty($content['is_turut_mengundang_active']))
                    <div class="mt-32 p-1 bg-gradient-to-r from-transparent via-brand-gold/30 to-transparent">
                        <div class="bg-brand-dark py-12 px-6">
                            <p class="text-[10px] tracking-[0.6em] uppercase text-brand-gold mb-10 font-bold text-center">Turut Mengundang</p>
                            <div class="flex flex-wrap justify-center gap-x-10 gap-y-6 max-w-4xl mx-auto">
                                @if(!empty($content['turut_mengundang_groom']))
                                    @foreach($content['turut_mengundang_groom'] as $t_groom)
                                        <span class="text-white/90 text-sm font-light tracking-wide hover:text-brand-gold transition-colors">{{ $t_groom }}</span>
                                    @endforeach
                                @endif
                                @if(!empty($content['turut_mengundang_bride']))
                                    @foreach($content['turut_mengundang_bride'] as $t_bride)
                                        <span class="text-white/90 text-sm font-light tracking-wide hover:text-brand-gold transition-colors">{{ $t_bride }}</span>
                                    @endforeach
                                @endif
                            </div>
                        </div>
        ...
        <section class="py-20 px-6 bg-brand-dark relative overflow-hidden">
        ...
        <section id="rsvp-modal" class="fixed inset-0 z-[100] invisible transition-all duration-500 overflow-hidden">
            ...
            <div id="rsvp-content" class="absolute bottom-0 left-0 right-0 bg-brand-dark rounded-t-[3.5rem] border-t border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,0.5)] transform translate-y-full transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] px-6 pb-12 pt-4 max-w-2xl mx-auto">
                ...
                <form id="rsvpForm" class="space-y-5 text-left">
                    <div class="group">
                    ...
                    <div class="flex gap-4 pt-4">
                        ...
                        <button type="button" onclick="submitRSVPDummy()" id="btnSubmitRsvp" class="flex-[2] py-4 bg-brand-gold text-brand-dark rounded-[1.5rem] font-bold text-[10px] uppercase tracking-widest transition-all shadow-[0_10px_30px_rgba(197,160,101,0.3)] hover:scale-[1.02] active:scale-95">
    ...
    <nav id="bottom-nav" class="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 floating-nav rounded-[2rem] transition-all duration-1000 translate-y-32">
        <ul class="flex justify-around items-center h-20 w-[340px] md:w-[450px] px-4">
            ...
            <li><a href="javascript:void(0)" onclick="openRSVP()" class="nav-link flex flex-col items-center text-[10px] uppercase tracking-[0.2em] font-bold gap-2 text-white/50 hover:text-brand-gold transition-colors"><div class="relative"><i class="fa-solid fa-paper-plane text-lg"></i><span class="absolute -top-1 -right-1 w-2 h-2 bg-brand-gold rounded-full animate-pulse"></span></div><span class="hidden md:block">RSVP</span></a></li>
        </ul>
    </nav>
    ...
    <script>
        ...
        function selectAttendance(status) {
            ...
            if (status === 'Hadir') {
                ...
                btnHadir.classList.add('shadow-lg', 'shadow-brand-gold/20');
                ...
                btnAbsen.classList.add('shadow-lg', 'shadow-brand-gold/20');
        ...
        function selectGuestCount(count, btnElement) {
            ...
            btnElement.classList.add('bg-brand-gold', 'text-brand-dark');
        }
        ...
        function submitRSVPDummy() {
            ...
            setTimeout(() => {
                ...
                if(document.getElementById('wishes-container')){
                    ...
                    newCard.className = 'wish-card bg-white/5 p-6 rounded-[1.5rem] border border-white/10 animate-fade-in-up transition-all duration-500 hover:bg-white/10 hover:border-brand-gold/30';
                    newCard.innerHTML = `<div class="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-2"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-full bg-brand-gold/20 flex items-center justify-center border border-brand-gold/30"><i class="fa-solid fa-user text-[10px] text-brand-gold"></i></div><h5 class="text-sm font-serif font-bold text-white tracking-wide">${document.getElementById('input-nama-rsvp').value || 'Tamu'}</h5></div><span class="text-[8px] text-brand-gold/60 italic uppercase tracking-[0.2em] flex items-center"><i class="fa-regular fa-clock mr-1.5"></i> Baru saja</span></div><div class="relative"><i class="fa-solid fa-quote-left absolute -top-2 -left-2 text-brand-gold/10 text-2xl"></i><p class="text-[13px] text-white/70 leading-relaxed font-light italic pl-4">${document.getElementById('input-pesan-rsvp').value}</p></div>`;
        ...
        // Load Ucapan
        const allWishes = [
            { nama: "M. Fauzan", pesan: "Selamat menempuh hidup baru Romeo & Juliet!", waktu: "2 Jam yang lalu" },
            { nama: "Jati Nugroho", pesan: "Lancar-lancar sampai hari H ya! Doa terbaik.", waktu: "5 Jam yang lalu" },
        ...
        function renderWishes() {
            ...
            wishesToDisplay.forEach(wish => {
                ...
                card.innerHTML = `<div class="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-2"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-full bg-brand-gold/20 flex items-center justify-center border border-brand-gold/30"><i class="fa-solid fa-user text-[10px] text-brand-gold"></i></div><h5 class="text-sm font-serif font-bold text-white tracking-wide">${wish.nama}</h5></div><span class="text-[8px] text-brand-gold/60 italic uppercase tracking-[0.2em] flex items-center"><i class="fa-regular fa-clock mr-1.5"></i> ${wish.waktu}</span></div><div class="relative"><i class="fa-solid fa-quote-left absolute -top-2 -left-2 text-brand-gold/10 text-2xl"></i><p class="text-[13px] text-white/70 leading-relaxed font-light italic pl-4">${wish.pesan}</p></div>`;
