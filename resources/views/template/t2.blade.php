...
<body class="bg-brand-dark text-white font-sans antialiased relative selection:bg-brand-gold selection:text-brand-dark">
    ...
    {{-- COVER PAGE --}}
    <div id="cover-page"
        class="fixed inset-0 z-50 flex items-center justify-center w-screen h-screen bg-brand-dark overflow-hidden transition-transform duration-1000 ease-in-out">
        <div class="absolute inset-0 z-0">
            {{-- MENGGUNAKAN COVER IMAGE DINAMIS --}}
            <div
                class="absolute inset-0 opacity-30 animate-slow-zoom bg-[url('{{ $coverImg }}')] bg-cover bg-center">
            </div>
        ...
        <div class="hidden lg:block absolute inset-0 z-10 pointer-events-none">
            ...
            <div class="absolute left-20 top-1/2 -translate-y-1/2 -rotate-90 origin-left">
                <p class="text-[10px] tracking-[1em] uppercase text-brand-gold/40 font-light whitespace-nowrap">The
                    Wedding of {{ $pria }} & {{ $wanita }} — {{ $coverDate }}</p>
            </div>
        </div>
        ...
        <div
            ...
            <div
                ...
                <p class="text-[11px] text-brand-muted mb-4 tracking-widest uppercase italic font-light">
                    {{ $content['cover_greeting'] ?? 'Dear Bapak/Ibu/Saudara/i' }}</p>
                <h2 id="guest-name"
    ...
    <main id="main-content" class="min-h-screen pb-28 opacity-0 transition-opacity duration-1000 bg-brand-dark">
        ...
        <section id="home"
            ...
            <div class="absolute inset-0 z-0">
                <div class="absolute inset-0 bg-[url('{{ $coverImg }}')] bg-cover bg-center bg-fixed opacity-20">
                </div>
            ...
            <div class="relative z-10 max-w-4xl w-full">
                <div class="mb-12 md:mb-20 animate-fade-in">
                    ...
                    <p
                        class="text-xs md:text-sm text-brand-muted max-w-lg mx-auto leading-loose tracking-wide font-light px-4">
                        Dengan memohon rahmat dan ridho Allah SWT, kami mengundang Bapak/Ibu/Saudara/i untuk menghadiri
                        momen bahagia pernikahan kami.
                    </p>
                </div>
                        ...
                        <div class="text-center md:text-right order-2 md:order-1 flex-1">
                            ...
                            @if (!empty($firstPerson['ig']))
                                <a href="https://instagram.com/{{ $firstPerson['ig'] }}" target="_blank"
                                    class="inline-flex items-center gap-2 mt-6 text-brand-gold/60 hover:text-brand-gold transition text-xs font-semibold tracking-widest uppercase">
                        ...
                        <div class="mt-32 p-1 bg-gradient-to-r from-transparent via-brand-gold/30 to-transparent">
                            <div class="bg-brand-dark py-12 px-6">
                                <p
                                    ...
                                    Turut Mengundang</p>
                                <div
                                    class="flex flex-wrap justify-center text-center gap-x-10 gap-y-6 max-w-4xl mx-auto">
                                    @foreach ($content['turut_mengundang'] as $nama)
                                        @if (trim($nama) !== '')
                                            <span
                                                class="text-white/90 text-sm font-light tracking-wide hover:text-brand-gold transition-colors">{{ trim($nama) }}</span>
                                        @endif
                                    @endforeach
                                </div>
                            </div>
        ...
        @if (!empty($content['is_story_active']) && !empty($content['love_stories']))
            <section id="story"
                class="py-24 px-6 bg-brand-dark relative overflow-hidden border-t border-brand-gold/20">
                <div
                ...
                <div class="max-w-6xl mx-auto relative z-10">
                    ...
                    <div class="relative space-y-20 md:space-y-32">
                        ...
                        @foreach ($content['love_stories'] as $index => $story)
                            ...
                            <div
                                class="relative grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center group {{ $index > 2 ? 'hidden extra-story' : '' }}">
                                <div
                                ...
                                <div
                                    class="relative {{ $isEven ? 'order-1 md:order-1 flex justify-center md:justify-end md:pr-10 lg:pr-16' : 'order-1 md:order-2 flex justify-center md:justify-start md:pl-10 lg:pl-16' }}">
                                ...
                                <div
                                    class="{{ $isEven ? 'order-2 md:order-2 text-center md:text-left md:pl-10 lg:pl-16 flex flex-col items-center md:items-start' : 'order-2 md:order-1 text-center md:text-right md:pr-10 lg:pr-16 flex flex-col items-center md:items-end' }}">
                                    <span
                    ...
                    @if (count($content['love_stories']) > 3)
                        <div class="mt-24 text-center">
                            <button id="btn-read-more" onclick="toggleStories()"
                                class="group relative px-12 py-4 bg-transparent border border-brand-gold/50 text-brand-gold rounded-full overflow-hidden transition-all duration-500 hover:border-brand-gold active:scale-95 shadow-[0_0_20px_rgba(212,175,55,0.1)]">
                                <span
                                    class="relative z-10 text-[11px] font-bold uppercase tracking-[0.3em] group-hover:text-brand-dark transition-colors duration-500">Baca
                                    Selengkapnya</span>
                                <div
                                    class="absolute inset-0 bg-brand-gold translate-y-full group-hover:translate-y-0 transition-transform duration-500">
                                </div>
                            </button>
                        </div>
                    @endif
        ...
        @if (!empty($content['is_event_active']))
            <section id="lokasi"
                ...
                <div class="max-w-6xl mx-auto relative z-10">
                    ...
                    <div
                        class="grid grid-cols-1 {{ count($content['events'] ?? []) > 0 ? 'md:grid-cols-2' : '' }} gap-8 lg:gap-12 items-stretch">

                        {{-- AKAD NIKAH STATIS --}}
                        @if (!empty($content['akad_location']))
                            <div
                                class="glass-card p-10 md:p-14 rounded-[3rem] border border-white/10 relative group hover:border-brand-gold/40 transition-all duration-500 flex flex-col items-center text-center h-full">
                                <div
                        ...
                        @if (!empty($content['events']))
                            @foreach ($content['events'] as $event)
                                <div
                                    ...
                                    <p class="text-brand-gold text-[10px] tracking-[0.4em] uppercase font-bold mb-8">
                                        Grand Celebration</p>
        ...
        @if (!empty($content['is_guest_info_active']))
            <section class="py-20 px-6 bg-brand-dark relative overflow-hidden border-t border-brand-gold/20">
                <div
                    class="max-w-3xl mx-auto text-center border border-brand-gold/20 rounded-[3rem] p-10 md:p-16 relative bg-white/5 backdrop-blur-sm shadow-xl">
                    ...
                    @if (!empty($content['enable_dresscode']) && !empty($content['dresscode']))
                        <div class="mb-10">
                            <p class="text-[10px] uppercase tracking-widest text-brand-gold/60 font-bold mb-2">
                                Dresscode</p>
                    ...
                    @if (!empty($content['enable_health_protocol']))
                        <div class="pt-6 border-t border-brand-gold/20 w-4/5 mx-auto">
                            <p class="text-[10px] uppercase tracking-widest text-brand-gold/60 font-bold mb-6">Protokol
                                Kesehatan</p>
                            <div class="flex justify-center gap-8 text-brand-gold text-3xl">
                                <div class="flex flex-col items-center gap-3 group"><i
                                        class="fa-solid fa-head-side-mask group-hover:scale-110 transition-transform"></i><span
                                        class="text-[9px] text-white/50 uppercase tracking-widest">Masker</span></div>
                                <div class="flex flex-col items-center gap-3 group"><i
                                        class="fa-solid fa-hands-bubbles group-hover:scale-110 transition-transform"></i><span
                                        class="text-[9px] text-white/50 uppercase tracking-widest">Cuci Tangan</span>
                                </div>
                                <div class="flex flex-col items-center gap-3 group"><i
                                        class="fa-solid fa-people-arrows group-hover:scale-110 transition-transform"></i><span
                                        class="text-[9px] text-white/50 uppercase tracking-widest">Jaga Jarak</span>
                                </div>
                            </div>
                        </div>
                    @endif
        ...
        @if (!empty($content['is_gallery_active']))
                    ...
                    @if ($invitation->galleries->count() > 0)
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5" id="photo-grid">
                            @foreach ($invitation->galleries->where('type', 'photo') as $idx => $photo)
                                <div class="group relative aspect-square rounded-2xl md:rounded-[2rem] overflow-hidden cursor-pointer shadow-xl border border-white/5"
            ...
            <div id="lightbox"
                class="fixed inset-0 z-[200] hidden flex-col items-center justify-center bg-brand-dark/95 backdrop-blur-xl p-4 transition-all duration-500">
                <button onclick="closeLightbox()"
                    class="absolute top-6 right-6 text-white/50 hover:text-brand-gold transition-colors text-3xl z-[110]">
                    <i class="fa-solid fa-xmark"></i>
                </button>
                <div class="relative w-full max-w-5xl flex items-center justify-center group">
                    <button onclick="prevImg()"
                        class="absolute left-0 md:-left-20 text-white/30 hover:text-brand-gold transition-all text-4xl hidden md:block group-hover:translate-x-2"><i
                    ...
                    <button onclick="nextImg()"
                        class="absolute right-0 md:-right-20 text-white/30 hover:text-brand-gold transition-all text-4xl hidden md:block group-hover:-translate-x-2"><i
                            class="fa-solid fa-chevron-right"></i></button>
                </div>
                <p class="mt-8 text-brand-gold tracking-[0.3em] text-[10px] uppercase font-bold">Image <span
                        id="current-count">1</span> of <span id="total-count">5</span></p>
            </div>
        @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
                        @foreach ($content['banks'] as $idx => $bank)
                            <div
                                ...
                                <div class="flex flex-col items-center">
                                    <div
                                        class="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center shadow-sm border border-brand-gold/30 mb-8 text-brand-gold text-2xl group-hover:scale-110 transition-transform">
                                        <i class="fa-solid fa-building-columns"></i>
                                    </div>
                                    <p class="text-sm uppercase tracking-widest text-white mb-6 font-bold">
                                        {{ $bank['name'] }}</p>
                                    <p class="text-[9px] uppercase tracking-[0.4em] text-brand-gold/60 mb-3 font-bold">
                                        Nomor Rekening</p>
                                    <h3 id="rek-{{ $idx }}"
                                        class="text-3xl font-serif font-bold text-white mb-2 tracking-widest group-hover:text-brand-gold transition-colors duration-500">
            ...
            <section id="guest-stats"
                ...
                <div class="max-w-4xl mx-auto relative z-10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                        <div
                            class="glass-stat p-8 rounded-[2.5rem] flex items-center gap-6 transition-all duration-500 hover:border-brand-gold/30 group">
                            <div
                                class="w-16 h-16 bg-brand-gold/10 rounded-2xl flex items-center justify-center shrink-0 border border-brand-gold/20 group-hover:bg-brand-gold group-hover:text-brand-dark transition-all duration-500">
                                <i
                                    class="fa-solid fa-user-check text-brand-gold text-2xl group-hover:text-brand-dark"></i>
                            </div>
                            <div>
                                <h4 id="total-attendance"
                                    class="text-4xl md:text-5xl font-serif font-bold text-white mb-1">0</h4>
                                <p class="text-[9px] uppercase tracking-[0.2em] text-brand-gold font-bold">Tamu Akan
                                    Hadir</p>
                            </div>
                        </div>
                    ...
                    <div class="mt-12 glass-stat rounded-[3rem] border border-white/10 p-2 overflow-hidden shadow-2xl">
                        ...
                        <div id="wishes-container"
                            class="max-h-[500px] overflow-y-auto scroll-custom px-6 md:px-10 py-4 space-y-6">
                            {{-- Diisi Oleh Script JS --}}
                        </div>

                        <div class="p-8 text-center border-t border-white/5">
                            <button id="btn-load-more" onclick="loadMoreWishes()"
                                class="group relative px-10 py-3.5 bg-transparent border border-brand-gold/40 text-brand-gold rounded-full overflow-hidden transition-all duration-500 hover:border-brand-gold">
                                <span
                                    class="relative z-10 text-[10px] font-bold uppercase tracking-widest group-hover:text-brand-dark transition-colors">Lihat
                                    Semua Pesan</span>
                                <div
                                    class="absolute inset-0 bg-brand-gold translate-y-full group-hover:translate-y-0 transition-transform duration-500">
                                </div>
        ...
        <section id="rsvp-modal" class="fixed inset-0 z-[100] invisible transition-all duration-500 overflow-hidden">
            ...
            <div id="rsvp-content"
                class="absolute bottom-0 left-0 right-0 bg-brand-dark rounded-t-[3.5rem] border-t border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,0.5)] transform translate-y-full transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] px-6 pb-12 pt-4 max-w-2xl mx-auto">
                ...
                <form id="rsvpForm" class="space-y-4 text-left" onsubmit="submitRSVP(event)">
                    <div class="group">
                        <label
                            ...
                            Lengkap</label>
                        <input type="text" id="input-nama-rsvp" name="name" placeholder="Tulis nama Anda..."
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <button type="button" onclick="selectAttendance('Hadir')" id="btn-hadir"
                            class="py-4 rounded-[1.5rem] border border-white/10 text-[11px] font-bold uppercase tracking-widest transition-all duration-300 bg-white/5 text-white/50 hover:border-brand-gold/50">
                    ...
                    <div class="group">
                        <label
                            ...
                            & Ucapan</label>
                        <textarea id="input-pesan-rsvp" name="message" rows="4" placeholder="Berikan doa terbaik Anda untuk kami..."
                    ...
                    <div class="flex gap-4 pt-4">
                        <button type="button" onclick="closeRSVP()"
                            class="flex-1 py-4 bg-white/5 text-white/40 rounded-[1.5rem] font-bold text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all">Batal</button>
                        <button type="submit" id="btnSubmitRsvp"
                            class="flex-[2] py-4 bg-brand-gold hover:bg-brand-charcoal text-white rounded-2xl font-semibold text-sm transition-all shadow-lg shadow-brand-gold/20">
                            Kirim RSVP
                        </button>
                    </div>
                </form>
        ...
        <footer class="py-12 px-6 bg-brand-dark border-t border-white/10 text-center">
            <div class="max-w-md mx-auto">
                ...
                <div
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 shadow-sm">
                    ...
                    <a href="https://instagram.com/ruangrestu.undangan" target="_blank" rel="noopener noreferrer"
                        class="text-[11px] font-semibold text-brand-gold hover:text-white transition-colors flex items-center gap-1">
    ...
    <div id="fab-container"
        ...
        <div class="relative flex items-center group">
            <div id="music-info"
                class="absolute right-full mr-3 px-3 py-1 bg-brand-dark/90 backdrop-blur border border-brand-gold/30 rounded-lg text-brand-gold text-xs whitespace-nowrap shadow-md opacity-0 translate-x-4 pointer-events-none transition-all duration-500 group-hover:opacity-100 group-hover:translate-x-0">
                🎵 Memutar Musik
            </div>
            <button id="btn-music" onclick="toggleMusic()"
                class="w-11 h-11 bg-brand-dark backdrop-blur border border-brand-gold/40 rounded-full flex items-center justify-center text-brand-gold shadow-lg hover:bg-brand-gold hover:text-brand-dark transition-all">
    ...
    <nav id="bottom-nav"
        ...
        <ul class="flex justify-around items-center h-20 w-[340px] md:w-[450px] px-4">
            <li><a href="#home"
                    class="nav-link flex flex-col items-center text-[10px] uppercase tracking-[0.2em] font-bold gap-2 text-white/50 hover:text-brand-gold transition-colors"><i
                        class="fa-solid fa-house-chimney text-lg"></i><span class="hidden md:block">Home</span></a>
            </li>
            @if (!empty($content['is_gallery_active']))
                <li><a href="#gallery"
                        class="nav-link flex flex-col items-center text-[10px] uppercase tracking-[0.2em] font-bold gap-2 text-white/50 hover:text-brand-gold transition-colors"><i
                            class="fa-solid fa-cloud-sun text-lg"></i><span class="hidden md:block">Gallery</span></a>
                </li>
            @endif
            @if (!empty($content['is_event_active']))
                <li><a href="#lokasi"
                        class="nav-link flex flex-col items-center text-[10px] uppercase tracking-[0.2em] font-bold gap-2 text-white/50 hover:text-brand-gold transition-colors"><i
                            class="fa-solid fa-compass text-lg"></i><span class="hidden md:block">Venue</span></a>
                </li>
            @endif
            <li><a href="javascript:void(0)" onclick="openRSVP()"
                    class="nav-link flex flex-col items-center text-[10px] uppercase tracking-[0.2em] font-bold gap-2 text-white/50 hover:text-brand-gold transition-colors">
                    <div class="relative"><i class="fa-solid fa-paper-plane text-lg"></i><span
                            class="absolute -top-1 -right-1 w-2 h-2 bg-brand-gold rounded-full animate-pulse"></span>
                    </div><span class="hidden md:block">RSVP</span>
                </a></li>
        </ul>
    </nav>
