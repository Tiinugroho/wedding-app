...
<head>
    ...
    <style>
        ...
        .vignette {
            background: radial-gradient(circle at center, transparent 0%, #141414 100%);
        }
...
<body
    class="bg-netflix-dark text-netflix-light font-sans selection:bg-netflix-red selection:text-white relative cover-locked">
    ...
    <div id="cover-page"
        class="fixed inset-0 z-[100] flex flex-col items-center justify-center w-screen h-screen bg-netflix-dark transition-all duration-700">
        ...
        <div class="flex flex-col items-center justify-center transform transition-transform duration-500">
            ...
            <div class="flex flex-wrap justify-center gap-6 md:gap-10">
                <button onclick="openInvitation()"
                    ...
                    <div
                        class="w-24 h-24 md:w-36 md:h-36 rounded-md overflow-hidden border-2 border-transparent group-hover:border-white transition-colors relative">
                        <img src="https://upload.wikimedia.org/wikipedia/commons/0/0b/Netflix-avatar.png"
                            class="w-full h-full object-cover" alt="Profile">
                        <div class="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors"></div>
                    </div>
                    <span id="guest-name"
                        class="text-netflix-gray group-hover:text-white transition-colors text-sm md:text-lg font-medium">{{ $guestNameDisplay }}</span>
                </button>
            </div>
            <button onclick="openInvitation()"
                class="mt-16 px-6 py-2 border border-netflix-gray text-netflix-gray hover:text-white hover:border-white uppercase tracking-widest text-xs font-medium transition-colors">
                Buka Undangan
            </button>
        </div>
    </div>
    ...
    <main id="main-content" class="min-h-screen opacity-0 transition-opacity duration-1000 pb-20">
        ...
        <nav class="fixed top-0 w-full z-50 bg-gradient-to-b from-black/80 to-transparent p-6 flex justify-between items-center transition-all duration-300"
            id="main-nav">
            <h1 class="font-bebas text-netflix-red text-3xl md:text-4xl tracking-widest drop-shadow-md">NikahNih</h1>
            <div class="flex items-center gap-4">
                <i
                    class="fa-solid fa-magnifying-glass text-white cursor-pointer hover:text-netflix-gray transition-colors"></i>
        ...
        <section id="home"
            class="relative h-[85vh] md:h-screen flex items-center justify-start px-6 md:px-16 pt-20">
            ...
            <div class="relative z-10 max-w-2xl mt-12 md:mt-0">
                <div class="flex items-center gap-2 mb-4">
                    <img src="https://upload.wikimedia.org/wikipedia/commons/0/08/Netflix_2015_logo.svg"
                        class="h-4 brightness-0 invert" alt="N">
                    <span class="text-[10px] md:text-xs font-bold tracking-[0.3em] text-white/80 uppercase">Original
                        Event</span>
                </div>

                <h2 class="text-6xl md:text-8xl font-bebas text-white tracking-wider mb-4 text-shadow leading-none">
                    {{ strtoupper($firstPerson['nickname']) }} <span class="text-netflix-red">&</span>
                    {{ strtoupper($secondPerson['nickname']) }}
                </h2>
                ...
                <div class="flex items-center gap-3 text-xs md:text-sm font-semibold mb-6">
                    ...
                    <span
                        class="px-1.5 py-0.5 border border-gray-500 text-white rounded-sm text-[10px] flex items-center gap-1"><i
                            class="fa-solid fa-closed-captioning"></i> HD</span>
                </div>
                ...
                <p class="text-sm md:text-base text-white/90 leading-relaxed mb-8 max-w-lg text-shadow font-light">
                    {!! nl2br(
                        e(
                            $content['quotes'] ??
                                'Dengan memohon rahmat Tuhan, dua pemeran utama kami bersiap mengikat janji suci. Sebuah perayaan cinta abadi yang tayang perdana secara eksklusif. Jadilah saksi di hari paling bahagia mereka.',
                        ),
                    ) !!}
                </p>
                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    <a href="javascript:void(0)" onclick="openRSVP()"
                        class="px-6 py-3 bg-white text-black rounded font-bold flex items-center justify-center gap-3 hover:bg-white/80 transition-colors">
                        <i class="fa-solid fa-play text-xl"></i> RSVP Now
                    </a>
                    <a href="#cerita"
                        class="px-6 py-3 bg-gray-500/50 text-white rounded font-bold flex items-center justify-center gap-3 hover:bg-gray-500/70 transition-colors backdrop-blur-sm">
        ...
        <section id="cast" class="px-6 md:px-16 py-12 relative z-10 -mt-20">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
                ...
                <div class="flex gap-6 items-center group {{ $groomFirst ? 'order-1' : 'order-2' }}">
                    <div
                        class="w-32 h-44 md:w-40 md:h-56 shrink-0 rounded-md overflow-hidden relative border border-white/10">
                    ...
                    <div>
                        <p class="text-netflix-gray text-xs uppercase tracking-widest mb-1 font-bold">
                            {{ $firstPerson['label'] }}</p>
                        <h4 class="text-2xl md:text-3xl font-bebas text-white tracking-wide mb-2">
                            {{ $firstPerson['name'] }}</h4>
                        <p class="text-sm text-netflix-gray font-light">{{ $firstPerson['gender_text'] }} dari Bapak
                            {{ $firstPerson['father'] }} & Ibu {{ $firstPerson['mother'] }}</p>
                        @if (!empty($firstPerson['ig']))
                            <a href="https://instagram.com/{{ str_replace('@', '', $firstPerson['ig']) }}"
                                target="_blank"
                                class="inline-flex items-center gap-2 mt-2 text-netflix-gray hover:text-white transition-colors">
                ...
                <div class="flex gap-6 items-center group {{ $groomFirst ? 'order-2' : 'order-1' }}">
                    ...
                    <div>
                        ...
                        <h4 class="text-2xl md:text-3xl font-bebas text-white tracking-wide mb-2">
                            {{ $secondPerson['name'] }}</h4>
                        <p class="text-sm text-netflix-gray font-light">{{ $secondPerson['gender_text'] }} dari Bapak
                            {{ $secondPerson['father'] }} & Ibu {{ $secondPerson['mother'] }}</p>
                        @if (!empty($secondPerson['ig']))
                            <a href="https://instagram.com/{{ str_replace('@', '', $secondPerson['ig']) }}"
                                target="_blank"
                                class="inline-flex items-center gap-2 mt-2 text-netflix-gray hover:text-white transition-colors">
            ...
            <section id="cerita" class="px-6 md:px-16 py-12 border-t border-netflix-gray/20 mt-8">
                ...
                <div class="space-y-6">
                    @foreach ($content['love_stories'] as $index => $story)
                        <div
                            class="episode-card flex flex-col md:flex-row gap-4 md:gap-6 items-start md:items-center p-4 hover:bg-white/5 rounded-lg transition-colors cursor-pointer border-b border-white/5 pb-6">
                            <h4 class="text-4xl font-bebas text-netflix-gray/50 hidden md:block">{{ $index + 1 }}
                            </h4>
                            <div
                                class="w-full md:w-48 h-28 shrink-0 rounded overflow-hidden relative episode-img-container border border-white/10">
                                @if (!empty($story['image']))
                                    <img src="{{ asset('storage/' . $story['image']) }}"
                                        class="episode-img w-full h-full object-cover transition-transform duration-500"
                                        alt="Ep {{ $index + 1 }}">
                                @else
                                    <div class="w-full h-full bg-[#333] flex items-center justify-center"><i
                                            class="fa-solid fa-heart text-netflix-gray/30 text-3xl"></i></div>
                                @endif
                                <div
                                    class="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 bg-black/40 transition-opacity">
                                    <i class="fa-regular fa-circle-play text-3xl text-white"></i>
                                </div>
                            </div>
                            <div class="flex-1">
                                <div class="flex justify-between items-start mb-2">
                                    <h5 class="font-bold text-white text-base">{{ $story['title'] ?? 'Our Journey' }}
                                    </h5>
                                    <span
                                        class="text-xs text-netflix-gray font-medium">{{ $story['year'] ?? '' }}</span>
                                </div>
        ...
        @if ($content['is_gallery_active'] ?? false)
            <section id="gallery" class="py-12 border-t border-white/5">
                <div class="px-6 md:px-16 flex items-center gap-8 border-b border-netflix-gray/30 mb-8">
                    <h3 class="text-xl md:text-2xl font-bold text-white border-b-4 border-netflix-red pb-4">Trailers &
                        More</h3>
                </div>
                ...
                @if (!empty($content['youtube_links'][0]))
                    ...
                    @if ($youtube_id)
                        <div class="px-6 md:px-16 mb-12">
                            <p class="text-xs font-bold text-netflix-red uppercase tracking-widest mb-4">Official
                                Trailer</p>
                            <div
                                class="relative w-full aspect-video rounded-md overflow-hidden bg-black shadow-2xl border border-white/10">
                                <iframe class="absolute inset-0 w-full h-full"
                                    src="https://www.youtube.com/embed/{{ $youtube_id }}?autoplay=0&controls=1"
                ...
                @if (isset($invitation->galleries) && $invitation->galleries->count() > 0)
                    <div class="relative group">
                        <div
                            class="flex overflow-x-auto gap-4 px-6 md:px-16 pb-8 snap-x snap-mandatory hide-scrollbar scroll-smooth">
                            @foreach ($invitation->galleries as $index => $gallery)
                                <div class="w-[70vw] md:w-[25vw] shrink-0 snap-start group/card cursor-pointer"
                                    onclick="openLightbox({{ $index }})">
                                    <div class="relative aspect-video rounded overflow-hidden border border-white/10">
                                        <img src="{{ asset('storage/' . $gallery->file_path) }}"
                                            class="gallery-img w-full h-full object-cover transition-transform duration-500 group-hover/card:scale-110">
                                        <div
        ...
        @if ($content['is_event_active'] ?? false)
            <section id="lokasi" class="px-6 md:px-16 py-12 border-t border-white/5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div
                        class="bg-[#181818] p-8 rounded-md border border-white/5 hover:border-netflix-red/50 transition-colors">
                        <div class="flex justify-between items-start mb-6">
                            <div>
                                <span
                                    class="text-netflix-red text-xs font-bold uppercase tracking-widest block mb-1">Part
                                    1</span>
                        ...
                        <ul class="space-y-4 mb-8 text-sm text-netflix-light">
                            <li class="flex items-center gap-3"><i
                                    class="fa-regular fa-calendar text-netflix-gray w-5"></i>
                                {{ \Carbon\Carbon::parse($akadDate)->translatedFormat('l, d F Y') }}</li>
                            <li class="flex items-center gap-3"><i
                                    class="fa-regular fa-clock text-netflix-gray w-5"></i>
                                {{ $akadTime }} - {{ $content['akad_time_end'] ?? 'Selesai' }}</li>
                            <li class="flex items-start gap-3"><i
                                    class="fa-solid fa-location-dot text-netflix-gray w-5 mt-1"></i>
                                <span>{{ $content['akad_location'] ?? 'Lokasi Akad' }}<br><span
                                        class="text-netflix-gray text-xs">{{ $content['akad_address'] ?? '' }}</span></span>
                            </li>
                        </ul>

                        @if (!empty($content['akad_map']))
                            <a href="{{ $content['akad_map'] }}" target="_blank"
                                class="block w-full py-3 bg-white/10 hover:bg-white/20 text-white text-center rounded text-sm font-semibold transition-colors">
                    ...
                    @foreach ($content['events'] ?? [] as $idx => $event)
                        <div
                            ...
                            <div class="flex justify-between items-start mb-6">
                                <div>
                                    <span
                                        class="text-netflix-red text-xs font-bold uppercase tracking-widest block mb-1">Part
                                        {{ $idx + 2 }}</span>
                                    <h4 class="text-2xl font-bebas tracking-wide text-white">
                            ...
                            <ul class="space-y-4 mb-8 text-sm text-netflix-light">
                                ...
                                <li class="flex items-start gap-3"><i
                                        class="fa-solid fa-location-dot text-netflix-gray w-5 mt-1"></i>
                                    <span>{{ $event['location'] ?? 'Lokasi Resepsi' }}<br><span
                                            class="text-netflix-gray text-xs">{{ $event['address'] ?? '' }}</span></span>
                                </li>
                            </ul>

                            @if (!empty($event['map']))
                                <a href="{{ $event['map'] }}" target="_blank"
                                    class="block w-full py-3 bg-netflix-red hover:bg-netflix-hover text-white text-center rounded text-sm font-semibold transition-colors">
                                    <i class="fa-solid fa-map-location-dot mr-2"></i> Get Directions
                                </a>
                            @endif
                        </div>
        ...
        @if (($content['is_livestream_active'] ?? false) && !empty($content['live_streams']))
            <section id="live-streaming"
                class="py-24 px-6 md:px-16 bg-[#141414] relative overflow-hidden border-t border-white/5">
                <div
                    class="absolute top-0 right-0 w-[500px] h-[500px] bg-red-600/5 blur-[120px] rounded-full pointer-events-none">
                </div>
                ...
                <div class="max-w-5xl mx-auto relative z-10">
                    <div class="text-left mb-10">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <span class="text-sm font-bold uppercase tracking-[0.3em] text-red-600">Live Virtual
                                Wedding</span>
                        </div>
                    ...
                    <div class="relative group">
                        <div id="streaming-display"
                            class="relative aspect-video w-full rounded-xl bg-black overflow-hidden border border-white/10 shadow-2xl transition-all duration-700">
                            <div class="absolute inset-0 bg-cover bg-center transition-all duration-1000 opacity-40 group-hover:scale-105"
                            ...
                            <div
                                class="relative h-full flex flex-col items-center justify-center p-6 text-white text-center">
                                <div class="mb-6 group-hover:scale-110 transition-transform duration-500">
                                    <i id="platform-icon"
                                        class="{{ $platformIcons[$firstStream['platform']]['icon'] ?? 'fa-solid fa-video' }} text-6xl md:text-8xl text-white drop-shadow-[0_0_20px_rgba(255,255,255,0.3)]"></i>
                                </div>

                                <h3 id="platform-title" class="text-2xl md:text-4xl font-bold mb-2 tracking-tight">
                                    {{ $platformIcons[$firstStream['platform']]['title'] ?? ucfirst($firstStream['platform']) }}
                                </h3>
                                <p id="platform-desc" class="text-netflix-gray text-sm md:text-lg mb-8 font-medium">
                                    Online Streaming</p>

                                <a id="platform-link" href="{{ $firstStream['link'] }}" target="_blank"
                                    class="flex items-center gap-3 px-8 md:px-10 py-3 md:py-4 bg-white text-black rounded font-bold text-sm md:text-lg hover:bg-white/80 transition-all active:scale-95 shadow-lg">
                    ...
                    <div class="mt-12 {{ count($content['live_streams']) <= 1 ? 'hidden' : '' }}">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            @foreach ($content['live_streams'] as $stream)
                                ...
                                <button
                                    onclick="switchPlatform('{{ $pData['title'] }}', 'Online Streaming', '{{ $pData['icon'] }}', '{{ $stream['link'] }}')"
                                    ...
                                    <span
                                        class="text-white font-bold text-xs uppercase tracking-wider mb-1">{{ $stream['platform'] }}</span>
        ...
        @if ($content['is_wishes_active'] ?? false)
            <section id="guest-stats"
                class="py-24 px-6 md:px-16 bg-[#141414] relative overflow-hidden border-t border-white/5">
                <div
                    class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-red-600/5 rounded-full blur-[120px] pointer-events-none">
                </div>
                ...
                <div class="max-w-5xl mx-auto relative z-10">
                    <div class="mb-16 text-center md:text-left">
                        ...
                        <div class="flex items-center justify-center md:justify-start gap-2">
                            ...
                            <p class="text-netflix-gray text-xs md:text-sm font-bold uppercase tracking-[0.3em]">
                                Trending Now</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-10 mb-16">
                        <div class="flex items-center gap-4 group">
                            <div class="relative">
                                <span class="text-8xl md:text-9xl font-black text-black leading-none"
                                    style="-webkit-text-stroke: 2px #555;">1</span>
                            </div>
                            <div
                                class="bg-netflix-gray/10 p-6 rounded-lg border border-white/5 flex-1 group-hover:bg-netflix-gray/20 transition-all">
                                <h4 id="total-attendance" class="text-4xl md:text-5xl font-bold text-white mb-1">
                                    {{ $totalAttendance }}</h4>
                                <p class="text-[10px] uppercase tracking-[0.2em] text-red-600 font-bold">Tamu Akan
                                    Hadir</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-4 group">
                            <div class="relative">
                                <span class="text-8xl md:text-9xl font-black text-black leading-none"
                                    style="-webkit-text-stroke: 2px #555;">2</span>
                            </div>
                            <div
                                class="bg-netflix-gray/10 p-6 rounded-lg border border-white/5 flex-1 group-hover:bg-netflix-gray/20 transition-all">
                                <h4 id="total-wishes" class="text-4xl md:text-5xl font-bold text-white mb-1">
                                    {{ $totalWishes }}</h4>
                                <p class="text-[10px] uppercase tracking-[0.2em] text-red-600 font-bold">Ucapan Hangat
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-black/40 rounded-xl border border-white/10 overflow-hidden shadow-2xl relative">
                        <div
                            class="flex items-center justify-between p-6 border-b border-white/10 bg-netflix-gray/5 relative z-10">
                        ...
                        <div id="wishes-container"
                            class="max-h-[500px] overflow-y-auto scroll-custom p-4 md:p-8 space-y-4 relative z-10">
                        </div>

                        <div
                            class="absolute bottom-0 left-0 w-full h-16 bg-gradient-to-t from-black to-transparent pointer-events-none z-20">
                        </div>
                    </div>
                </div>
            </section>
        ...
        @if ($content['is_gift_active'] ?? false)
            <section id="hadiah"
                class="py-24 px-6 md:px-16 bg-[#141414] relative overflow-hidden border-t border-white/5">
                <div
                    class="absolute top-1/2 left-0 w-[500px] h-[500px] bg-red-600/5 blur-[120px] rounded-full pointer-events-none">
                </div>
                ...
                <div class="max-w-5xl mx-auto relative z-10">
                    <div class="mb-16 md:text-center text-center">
                        <div class="flex items-center justify-center gap-3 mb-4">
                            ...
                            <span class="text-[10px] tracking-[0.5em] uppercase text-gray-400 font-black">Gift
                                Registry</span>
                            <span class="w-8 h-[2px] bg-red-600"></span>
                        </div>
                        <h2 class="text-4xl md:text-6xl font-black text-white mb-6 uppercase tracking-tighter">Kirim
                            Kado</h2>
                        <p class="text-sm md:text-base text-gray-400 font-medium leading-relaxed max-w-2xl mx-auto">
                            Kehadiran Anda adalah kado terbesar. Namun bagi Anda yang ingin mengirimkan tanda kasih
                            fisik, kami telah menyediakan alamat pengiriman dan daftar kebutuhan kami.
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-10">
                        @foreach ($content['banks'] ?? [] as $index => $bank)
                            ...
                            @endphp
                            <div
                                class="group relative bg-[#181818] rounded-xl overflow-hidden border border-white/5 transition-all duration-500 hover:scale-[1.03] hover:border-red-600/50 hover:shadow-[0_20px_40px_rgba(0,0,0,0.6)]">
                                <div class="p-8 md:p-10">
                                    <div class="flex justify-between items-center mb-12 h-10">
                                        @if ($logoPath)
                                            @if (str_starts_with($logoPath, 'http'))
                                                <img src="{{ $logoPath }}"
                                                    class="h-6 md:h-8 object-contain brightness-0 invert opacity-60 group-hover:opacity-100 transition-all"
                                                    alt="Bank Logo">
                                            @else
                                        ...
                                        @else
                                            <i
                                                class="fa-solid fa-building-columns text-3xl text-gray-500 group-hover:text-white transition-colors"></i>
                                        @endif
                                        <i
                                            class="fa-solid fa-circle-check text-red-600 opacity-0 group-hover:opacity-100 transition-all text-xl"></i>
                                    </div>

                                    <div class="space-y-1 mb-10">
                                        <p class="text-[10px] uppercase tracking-widest text-gray-500 font-bold">Nomor
                                            Rekening</p>
                                        <h3 id="rek-{{ $index }}"
                                            class="text-3xl md:text-4xl font-black text-white tracking-tight">
                                            {{ $bank['account_number'] }}</h3>
                                        <p class="text-sm text-gray-400 font-medium uppercase tracking-wide">a.n
                                            {{ $bank['account_name'] }}</p>
                                    </div>

                                    <button onclick="copyToClipboard('rek-{{ $index }}', this)"
                                        class="w-full py-4 bg-white text-black rounded font-black text-[11px] uppercase tracking-[0.2em] transition-all hover:bg-red-600 hover:text-white flex items-center justify-center gap-3">
                                        <i class="fa-regular fa-copy"></i> <span>Salin Nomor</span>
                                    </button>
                                </div>
                                <div
                                    class="absolute bottom-0 left-0 h-1 w-0 bg-red-600 transition-all duration-500 group-hover:w-full">
                                </div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    @if (!empty($content['alamat_kado']))
                        <div class="flex flex-col items-center gap-8">
                            <div
                                class="group relative p-10 bg-[#181818] rounded-2xl border border-white/5 max-w-2xl w-full transition-all duration-500 hover:border-red-600/30 hover:scale-[1.01] shadow-2xl">
                                <div
                                    class="w-14 h-14 bg-red-600/10 rounded-full flex items-center justify-center mx-auto mb-8 transition-transform group-hover:scale-110">
                                    <i class="fa-solid fa-truck-fast text-red-600 text-xl"></i>
                                </div>
                                <p
                                    class="text-[10px] text-center uppercase tracking-[0.4em] text-red-600 mb-4 font-black">
                                    Alamat Pengiriman</p>
                                <div id="alamat-kado"
                                    class="text-xl md:text-2xl text-white font-bold text-center leading-tight mb-10 tracking-tight">
                                    {!! nl2br(e($content['alamat_kado'])) !!}
                                </div>
                                <div class="flex flex-col sm:flex-row justify-center gap-4">
                                    <button onclick="copyToClipboardText('alamat-kado', this)"
                                        class="px-8 py-4 bg-white text-black rounded font-black text-[11px] uppercase tracking-widest transition-all hover:bg-gray-200 active:scale-95 flex items-center justify-center gap-2">
                                        <i class="fa-regular fa-copy"></i> Salin Alamat
                                    </button>
                                    @if (!empty($content['gifts']))
                                        <button onclick="toggleGiftModal(true)"
                                            class="px-8 py-4 bg-red-600 text-white rounded font-black text-[11px] uppercase tracking-widest transition-all hover:bg-red-700 active:scale-95 shadow-lg shadow-red-600/20 flex items-center justify-center gap-2">
                    ...
                    @if (!empty($content['gifts']))
                        <div id="gift-modal"
                            ...
                            <div
                                class="relative bg-[#181818] w-full max-w-lg rounded-xl border border-white/10 overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-slide-up">
                                    ...
                                    <div class="flex justify-between items-center">
                                        <div>
                                            <h3 class="text-2xl font-black text-white uppercase tracking-tighter">
                                                Wishlist Kami</h3>
                                            <p
                                                class="text-[10px] text-red-600 uppercase tracking-widest mt-1 font-black">
                                                Wedding Registry</p>
                                        </div>
                                        <button onclick="toggleGiftModal(false)"
                                            class="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 text-white hover:bg-white/10 transition-all">
                                ...
                                <div class="flex-1 overflow-y-auto p-6 custom-scrollbar space-y-4">
                                    @foreach ($content['gifts'] as $index => $gift)
                                        <div id="item-{{ $index }}"
                                            class="p-5 rounded-lg border border-white/5 bg-white/5 flex items-center justify-between gap-4 transition-all hover:bg-white/10 group">
                                            <div>
                                                <h4
                                                    class="text-sm font-black text-white uppercase tracking-wide group-hover:text-red-600 transition-colors">
                                                    {{ $gift['item_name'] }}</h4>
                                                <p
                                                    class="text-[10px] text-gray-500 font-bold mt-1 uppercase tracking-widest">
                                                    {{ $gift['description'] ?? '' }}</p>
                                            </div>
                                            <button
                                                onclick="confirmGift('item-{{ $index }}', '{{ $gift['item_name'] }}')"
                                                class="shrink-0 px-5 py-2.5 bg-red-600 text-white rounded text-[9px] font-black uppercase tracking-widest hover:bg-red-700 transition-all">
                                                Pilih
                                            </button>
                                        </div>
                                    @endforeach
                                ...
                                <div class="p-6 bg-black/40 text-center border-t border-white/5">
                                    <p class="text-[9px] text-gray-500 font-black uppercase tracking-[0.3em]">Thank You
                                        for Your Kindness</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div id="confirm-modal" class="fixed inset-0 z-[600] hidden flex items-center justify-center p-6">
                        <div class="absolute inset-0 bg-black/95 backdrop-blur-xl" onclick="closeConfirmModal()">
                        </div>
                        <div
                            class="relative bg-[#181818] w-full max-w-sm rounded-xl p-8 text-center shadow-2xl border border-white/10 animate-slide-up">
                            <div
                                class="w-16 h-16 bg-red-600/20 text-red-600 rounded-full flex items-center justify-center mx-auto mb-6">
                            ...
                            <div class="text-left space-y-4 mb-8">
                                <div>
                                    <label
                                        class="block text-[10px] uppercase tracking-widest text-netflix-gray font-bold mb-2">Nama
                                        Anda</label>
                                    <input type="text" id="input-gift-name"
                                        class="input-luxury w-full p-3 rounded text-sm placeholder-white/20"
                                        placeholder="Masukkan nama Anda">
                                </div>
                                <div>
                                    <label
                                        class="block text-[10px] uppercase tracking-widest text-netflix-gray font-bold mb-2">Jumlah
                                        Hadir (Opsional)</label>
                                    <input type="number" id="input-gift-pax" min="0" value="0"
                                        class="input-luxury w-full p-3 rounded text-sm placeholder-white/20 text-center"
                                        placeholder="0">
                            ...
                            <div class="flex flex-col gap-3">
                                <button id="final-confirm-btn"
                                    class="w-full py-4 bg-red-600 text-white rounded font-black text-[11px] uppercase tracking-widest transition-all hover:bg-red-700 shadow-lg shadow-red-600/20">
                                    Kirim Konfirmasi
                                </button>
                                <button onclick="closeConfirmModal()"
                                    class="w-full py-4 bg-transparent text-gray-500 font-black text-[10px] uppercase tracking-widest hover:text-white transition-all">
                                    Batalkan
                                </button>
                            </div>
                        </div>
                    ...
                    <div id="gift-toast"
                        class="fixed bottom-10 left-1/2 -translate-x-1/2 z-[700] px-10 py-4 bg-red-600 text-white rounded shadow-2xl text-[11px] font-black uppercase tracking-[0.2em] opacity-0 transition-all duration-500 pointer-events-none text-center">
                        Terima kasih!
                    </div>
                </div>
            </section>
        ...
        @if ($content['enable_qr_attendance'] ?? false)
            <section id="qr-tamu"
                ...
                <div class="max-w-4xl mx-auto text-center relative z-10">
                    <div class="mb-16">
                        <h2 class="text-4xl md:text-5xl font-black text-white uppercase tracking-tighter mb-4">Akses
                            Undangan</h2>
                    ...
                    <div class="flex justify-center">
                        <div
                            class="group relative p-10 bg-[#181818] rounded-xl border border-white/5 transition-all duration-700 hover:border-red-600/50 max-w-sm w-full mx-auto">
                            <div
                                class="relative bg-white p-6 rounded-md mb-8 inline-block shadow-[0_0_30px_rgba(229,9,20,0.1)]">
                                <img id="qr-image" src="" class="w-44 h-44 object-contain"
                                    alt="QR Code Tamu">
                            </div>
                            <div class="space-y-2 mb-6 text-center">
                                <span
                                    class="text-[10px] uppercase tracking-[0.5em] text-red-600 font-black block">Guest
                                    Identity</span>
                                <h3 id="guest-name-qr"
                                    class="text-2xl font-black text-white tracking-tighter uppercase leading-none">{{ $guestNameDisplay }}</h3>
                            </div>
                            <div
                                class="w-full py-3 bg-white/5 rounded border border-dashed border-white/10 text-center">
                                <p class="text-[9px] uppercase tracking-[0.3em] font-bold text-gray-500">E-Invitation
                                    Only</p>
                            </div>
                        </div>
                    </div>
        ...
        <footer class="py-20 px-6 bg-[#141414] border-t border-white/10 text-center relative overflow-hidden">
            <div class="max-w-4xl mx-auto relative z-10">
                <div class="mb-12 flex flex-col items-center">
                    <h2 class="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter select-none">
                        NikahNih</h2>
                </div>
                <div class="mb-16">
                    <p
                        class="text-sm md:text-base font-medium text-netflix-gray leading-relaxed max-w-2xl mx-auto italic">
                        "Merupakan suatu kehormatan apabila Bapak/Ibu/Saudara/i berkenan hadir dan memberikan doa
                        restu."
                    </p>
                </div>
                <div class="mb-16 space-y-2">
                    <p class="text-[10px] text-netflix-gray uppercase font-bold tracking-[0.3em]">Starring</p>
                    <h3 class="text-3xl font-bebas text-white tracking-wide">
                        {{ strtoupper($firstPerson['nickname']) }} & {{ strtoupper($secondPerson['nickname']) }}</h3>
                </div>
                <div class="flex justify-center mb-8">
                    <a href="https://www.instagram.com/ruangrestu.undangan" target="_blank"
                        class="text-[10px] font-bold text-netflix-gray hover:text-white uppercase tracking-widest transition-colors flex items-center gap-2">
    ...
    <div id="fab-container"
        ...
        <button id="btn-music" onclick="toggleMusic()"
            class="w-11 h-11 bg-white text-black rounded-full flex items-center justify-center shadow-xl hover:scale-110 transition-transform pointer-events-auto">
            <i class="fa-solid fa-music animate-pulse-slow" id="icon-music"></i>
        </button>
        <button id="btn-scroll" onclick="toggleAutoScroll()"
            class="w-11 h-11 bg-[#333] text-white rounded-full flex items-center justify-center shadow-xl hover:scale-110 transition-transform pointer-events-auto">
    ...
    <nav id="bottom-nav"
        ...
        <div
            class="bg-black/90 backdrop-blur-xl border border-white/10 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.8)]">
            <ul class="flex justify-around items-center h-16 w-[320px] md:w-[420px] px-2">
                <li class="relative group">
                    <a href="#home"
                        class="nav-link flex flex-col items-center justify-center w-16 h-full text-netflix-gray hover:text-white transition-all">
                        <i class="fa-solid fa-house text-lg"></i><span
                            class="text-[8px] uppercase font-bold tracking-tighter mt-1">Home</span>
                    </a>
                </li>
                @if ($content['is_gallery_active'] ?? false)
                    <li class="relative group">
                        <a href="#gallery"
                            class="nav-link flex flex-col items-center justify-center w-16 h-full text-netflix-gray hover:text-white transition-all">
                            <i class="fa-solid fa-clapperboard text-lg"></i><span
                                class="text-[8px] uppercase font-bold tracking-tighter mt-1">Gallery</span>
                        </a>
                    </li>
                @endif
                <li class="relative group">
                    <a href="#lokasi"
                        class="nav-link flex flex-col items-center justify-center w-16 h-full text-netflix-gray hover:text-white transition-all">
                        <i class="fa-solid fa-location-dot text-lg"></i><span
                            class="text-[8px] uppercase font-bold tracking-tighter mt-1">Venue</span>
                    </a>
                </li>
                @if ($content['is_wishes_active'] ?? false)
                    <li class="relative">
                        <a href="javascript:void(0)" onclick="openRSVP()"
                            class="flex flex-col items-center justify-center w-20 h-14 bg-netflix-red rounded transition-all duration-300 hover:bg-red-700 active:scale-95 shadow-lg">
    ...
    <section id="rsvp-modal"
        class="fixed inset-0 z-[1000] invisible transition-all duration-500 overflow-hidden flex items-end md:items-center justify-center">
        <div onclick="closeRSVP()"
            class="absolute inset-0 bg-black/95 backdrop-blur-sm opacity-0 transition-opacity duration-500"
            id="rsvp-overlay"></div>

        <div id="rsvp-content"
            class="relative w-full md:max-w-xl lg:max-w-2xl h-[92vh] md:h-auto max-h-[95vh] bg-[#181818] rounded-t-2xl md:rounded-2xl border-t md:border border-white/10 shadow-2xl transform translate-y-full transition-transform duration-500 ease-out flex flex-col">
            ...
            <div class="overflow-y-auto px-6 pb-10 pt-4 custom-scrollbar">
                ...
                <form id="form-rsvp" class="space-y-6">
                    <div class="space-y-2">
                        <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Nama
                            Lengkap</label>
                        <input type="text" id="input-nama-rsvp" placeholder="Masukkan nama Anda"
                            class="input-luxury w-full p-4 rounded text-sm outline-none" required>
                    </div>

                    <div class="space-y-3">
                        <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Konfirmasi
                            Kehadiran</label>
                        <input type="hidden" id="input-status" value="Hadir">
                        <div class="grid grid-cols-2 gap-3">
                            <button type="button" onclick="selectAttendance('Hadir')" id="btn-hadir"
                                class="py-3.5 rounded border border-red-600 text-xs font-bold uppercase tracking-widest transition-all bg-red-600 text-white">
                                <i class="fa-solid fa-check mr-2"></i> Hadir
                            </button>
                            <button type="button" onclick="selectAttendance('Tidak Hadir')" id="btn-absen"
                                class="py-3.5 rounded border border-white/20 text-xs font-bold uppercase tracking-widest transition-all bg-transparent text-white hover:border-white/50">
                    ...
                    <div id="guest-selection"
                        class="bg-white/5 p-5 rounded-lg border border-white/5 transition-all duration-300">
                        <label class="text-[10px] uppercase tracking-widest text-gray-400 mb-3 block font-bold">Jumlah
                            Tamu</label>
                        <div class="flex gap-2 mb-3">
                            <button type="button" onclick="setGuestCount(1)"
                                class="guest-btn flex-1 py-3 rounded bg-red-600 text-sm text-white font-bold transition-all">1</button>
                            <button type="button" onclick="setGuestCount(2)"
                                class="guest-btn flex-1 py-3 rounded bg-[#333] text-sm text-white font-bold hover:bg-[#444] transition-all">2</button>
                            <button type="button" onclick="setGuestCount(3)"
                                class="guest-btn flex-1 py-3 rounded bg-[#333] text-sm text-white font-bold hover:bg-[#444] transition-all">3</button>
                        ...
                        <div id="custom-pax-container" class="hidden">
                            <input type="number" id="custom-pax-input" min="4"
                                placeholder="Ketik jumlah spesifik..."
                                class="input-luxury w-full p-3 rounded text-sm text-center outline-none">
                        </div>
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Pesan</label>
                        <textarea id="input-pesan-rsvp" rows="3" placeholder="Tulis doa restu Anda..."
                            class="input-luxury w-full p-4 rounded text-sm outline-none resize-none" required></textarea>
                    </div>

                    <div class="flex flex-col gap-3 pt-2">
                        <button type="submit"
                            class="w-full py-4 bg-red-600 text-white rounded font-black text-xs uppercase tracking-widest shadow-lg shadow-red-600/20 active:scale-95 transition-transform">
                            Kirim Konfirmasi
                        </button>
                        <button type="button" onclick="closeRSVP()"
                            class="w-full py-3 bg-transparent text-gray-500 rounded font-bold text-[10px] uppercase tracking-widest hover:text-white transition-colors">
                            Kembali
                        </button>
                    </div>
                </form>
    ...
    <div id="lightbox"
        class="fixed inset-0 z-[2000] hidden flex-col items-center justify-center bg-[#141414] p-4 transition-all duration-500">
        <div class="w-full flex justify-between items-center p-6 absolute top-0 left-0 z-10">
            <span class="text-white font-bold tracking-widest text-sm"><span id="current-count">1</span> / <span
                    id="total-count">4</span></span>
            <button onclick="closeLightbox()" class="text-white hover:text-netflix-red transition-colors text-3xl"><i
        ...
        <div class="absolute bottom-10 flex gap-6 z-10">
            <button onclick="prevImg()"
                class="w-12 h-12 rounded-full border border-white text-white hover:bg-white hover:text-black flex items-center justify-center transition-colors"><i
    ...
    @if (session('success'))
        <div id="success-toast"
            class="fixed top-10 left-1/2 -translate-x-1/2 z-[3000] px-8 py-4 bg-green-600 text-white rounded font-bold shadow-2xl text-sm transition-all duration-500 flex items-center gap-3 animate-slide-up">
    ...
    <script>
        ...
        if (qrImage) qrImage.src =
            `https://api.qrserver.com/v1/create-qr-code/?size=200x200&color=e50914&bgcolor=ffffff&data=${encodeURIComponent(guestName)}`;
        ...
        function renderWishes() {
            ...
            allWishes.forEach(wish => {
                ...
                card.className =
                    'flex flex-col md:flex-row gap-4 p-4 rounded-lg bg-netflix-gray/5 border border-transparent hover:bg-netflix-gray/10 hover:border-white/10 transition-all';
                card.innerHTML = `
        ...
        async function sendRsvpData(data) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (elAtt) {
                        elAtt.innerText = countAttendance;
                        elAtt.classList.add('text-red-600');
                        setTimeout(() => elAtt.classList.remove('text-red-600'), 500);
                    }
                    if (elWish) {
                        elWish.innerText = countWishes;
                        elWish.classList.add('text-red-600');
                        setTimeout(() => elWish.classList.remove('text-red-600'), 500);
                    }
        ...
        function selectAttendance(status) {
            ...
            [btnHadir, btnAbsen].forEach(b => b.className =
                'py-3.5 rounded border border-white/20 text-xs font-bold uppercase tracking-widest transition-all bg-transparent text-white hover:border-white/50 active:scale-95'
                );

            if (status === 'Hadir') {
                btnHadir.className =
                    'py-3.5 rounded border border-red-600 text-xs font-bold uppercase tracking-widest transition-all bg-red-600 text-white shadow-lg shadow-red-600/20 active:scale-95';
        ...
        function confirmGift(id, name) {
            ...
            `Silakan isi data diri untuk konfirmasi kado <b>${name}</b>`;
        ...
        function processClaimGift(senderName, giftName, giftPax) {
            ...
            if (btn) {
                btn.outerHTML =
                    `<div class="text-[9px] text-green-500 uppercase tracking-widest font-bold flex items-center gap-2"><i class="fa-solid fa-check-circle"></i> Terpilih</div>`;
            }
        ...
        function switchPlatform(title, desc, iconClass, link) {
            ...
            setTimeout(() => {
                ...
                document.getElementById('platform-icon').className = iconClass +
                    ' text-6xl md:text-8xl text-white drop-shadow-[0_0_20px_rgba(255,255,255,0.3)]';
