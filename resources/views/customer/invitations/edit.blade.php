...
<form action="{{ route('customer.invitations.update', $invitation->id) }}" method="POST" enctype="multipart/form-data" onsubmit="document.getElementById('loading-overlay').classList.replace('hidden', 'flex');">
    @csrf
    @method('PUT')
        ...
        <div class="xl:col-span-8 space-y-8">
            ...
            <div class="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="mb-2">
                        ...
                        <div class="flex gap-4 mt-3">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="couple_order" value="groom_first" class="w-4 h-4 text-rOrange focus:ring-rOrange" {{ ($content['couple_order'] ?? 'groom_first') == 'groom_first' ? 'checked' : '' }}>
                                <span class="text-sm font-medium text-slate-700">Pria Dulu</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="couple_order" value="bride_first" class="w-4 h-4 text-rOrange focus:ring-rOrange" {{ ($content['couple_order'] ?? '') == 'bride_first' ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" name="cover_greeting" value="{{ old('cover_greeting', $content['cover_greeting'] ?? 'Kepada Yth.') }}" class="w-full py-3 px-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-rOrange" placeholder="Misal: Kepada Yth. / Dear / To:">
                    </div>
                </div>

                <div class="mt-6 border-t border-slate-100 pt-6">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Quotes / Kutipan Pembuka (Opsional)</label>
                    <textarea name="quotes" rows="3" class="w-full py-3 px-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-rOrange" placeholder="Contoh: Dan di antara tanda-tanda kekuasaan-Nya ialah Dia menciptakan untukmu isteri-isteri dari jenismu sendiri...">{{ old('quotes', $content['quotes'] ?? '') }}</textarea>
                </div>
                ...
                <div class="mt-6 border-t border-slate-100 pt-6">
                    ...
                    <p class="text-xs text-slate-500 mb-3">Jika dikosongkan, sistem akan otomatis mengambil foto pertama dari Album Galeri.</p>
                    <div class="flex items-center gap-4">
                        @if (!empty($content['cover_image']))
                        <img src="{{ asset('storage/' . $content['cover_image']) }}" class="w-16 h-16 rounded-xl object-cover shadow-sm border border-slate-200">
                        @endif
                        <input type="file" name="cover_image" accept="image/*" class="w-full py-2 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm">
                    </div>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 mb-6">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer group">
                        <span class="mr-3 text-sm font-bold text-slate-500 group-hover:text-slate-700 transition-colors">Tampilkan</span>
            ...
            <div class="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 mb-6">
                    ...
                    <div class="flex items-center gap-4">
                        <button type="button" onclick="addEventRow()" class="px-4 py-2 bg-slate-900 text-white text-xs font-bold rounded-xl hover:bg-slate-800 transition shadow-sm">+ Tambah Resepsi</button>
                        <label class="relative inline-flex items-center cursor-pointer shrink-0">
                            <span class="mr-3 text-sm font-bold text-slate-500 hidden sm:block">Tampilkan</span>
                            <input type="checkbox" name="is_event_active" value="1" class="sr-only peer" {{ ($content['is_event_active'] ?? true) ? 'checked' : '' }}>
                            <div class="w-11 h-6 bg-slate-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] sm:after:left-[auto] sm:after:right-[24px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-rOrange"></div>
                        </label>
                    </div>
                </div>

                <div class="space-y-6 transition-opacity duration-300 {{ empty($content['is_event_active']) && isset($content['is_event_active']) ? 'opacity-40 pointer-events-none' : '' }}">
                    ...
                    <div class="p-5 bg-orange-50/50 rounded-2xl border border-orange-100 relative">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="akad_date" value="{{ old('akad_date', $content['akad_date'] ?? '') }}" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange">
                            </div>
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <input type="time" name="akad_time" value="{{ old('akad_time', $safeAkadTime) }}" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange" required>

                                    <span class="text-slate-400 font-bold">-</span>

                                    @php $safeAkadTimeEnd = !empty($content['akad_time_end']) ? substr(trim($content['akad_time_end']), 0, 5) : ''; @endphp
                                    <input type="time" name="akad_time_end" value="{{ old('akad_time_end', $safeAkadTimeEnd) }}" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange">
                                </div>
                            </div>

                            <div class="md:col-span-2"><label class="block text-xs font-bold text-slate-600 mb-1">Nama Tempat/Gedung</label><input type="text" name="akad_location" value="{{ old('akad_location', $content['akad_location'] ?? '') }}" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange" placeholder="Contoh: Masjid Raya Pekanbaru"></div>
                            <div class="md:col-span-2"><label class="block text-xs font-bold text-slate-600 mb-1">Alamat Lengkap</label><textarea name="akad_address" rows="2" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange" placeholder="Contoh: Jl. Senapelan No. 128, Riau">{{ old('akad_address', $content['akad_address'] ?? '') }}</textarea></div>
                            <div class="md:col-span-2"><label class="block text-xs font-bold text-slate-600 mb-1">Link Google Maps</label><input type="url" name="akad_map" value="{{ old('akad_map', $content['akad_map'] ?? '') }}" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange" placeholder="https://maps.google.com/..."></div>
                        </div>
                    </div>
                    ...
                    <div id="event-wrapper" class="space-y-6">
                        ...
                        'time_end' => $content['resepsi_time_end'] ?? '',
                        ...
                        if (empty($eventsData)) {
                        $eventsData[] = [
                        'title' => 'Resepsi Pernikahan', 'date' => '', 'time' => '', 'time_end' => '', 'location' => '', 'address' => '', 'map' => '',
                        ];
                        }
                        @endphp
                        ...
                        <div class="p-5 bg-blue-50/50 rounded-2xl border border-blue-100 relative event-item">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                                <div class="md:col-span-2">
                                    ...
                                    <input type="text" name="events[{{ $key }}][title]" value="{{ $event['title'] ?? '' }}" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange" placeholder="Contoh: Resepsi Pernikahan / Unduh Mantu">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-600 mb-1">Tanggal</label>
                                    <input type="date" name="events[{{ $key }}][date]" value="{{ $event['date'] ?? '' }}" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange">
                                </div>

                                <div>
                                    <label class="block text-xs font-bold text-slate-600 mb-1">Waktu Acara (Mulai - Selesai)</label>
                                    <div class="flex items-center gap-2">
                                        <input type="time" name="events[{{ $key }}][time]" value="{{ $safeEventTime }}" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange" required>
                                        <span class="text-slate-400 font-bold">-</span>
                                        <input type="time" name="events[{{ $key }}][time_end]" value="{{ $safeEventTimeEnd }}" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange">
                                    </div>
                                </div>

                                <div class="md:col-span-2"><label class="block text-xs font-bold text-slate-600 mb-1">Nama Tempat/Gedung</label><input type="text" name="events[{{ $key }}][location]" value="{{ $event['location'] ?? '' }}" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange" placeholder="Contoh: Grand Ballroom Hotel"></div>
                                <div class="md:col-span-2"><label class="block text-xs font-bold text-slate-600 mb-1">Alamat Lengkap</label><textarea name="events[{{ $key }}][address]" rows="2" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange" placeholder="Contoh: Pekanbaru, Riau">{{ $event['address'] ?? '' }}</textarea></div>
                                <div class="md:col-span-2"><label class="block text-xs font-bold text-slate-600 mb-1">Link Google Maps (Opsional)</label><input type="url" name="events[{{ $key }}][map]" value="{{ $event['map'] ?? '' }}" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange" placeholder="https://maps.google.com/..."></div>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 mb-6">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer shrink-0">
                        <span class="mr-3 text-sm font-bold text-slate-500 hidden sm:block">Tampilkan</span>
                        <input type="checkbox" name="is_guest_info_active" value="1" class="sr-only peer" {{ ($content['is_guest_info_active'] ?? true) ? 'checked' : '' }}>
                        <div class="w-11 h-6 bg-slate-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] sm:after:left-[auto] sm:after:right-[24px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-rOrange"></div>
                    </label>
                </div>

                <div class="space-y-6 transition-opacity duration-300 {{ empty($content['is_guest_info_active']) && isset($content['is_guest_info_active']) ? 'opacity-40 pointer-events-none' : '' }}">
                ...
                <div class="p-5 bg-blue-50/50 rounded-2xl border border-blue-100 relative event-item mt-4 animate-fade-in">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-600 mb-1">Waktu Acara (Mulai - Selesai)</label>
                            <div class="flex items-center gap-2">
                                <input type="time" name="events[${id}][time]" class="w-full py-2.5 px-4 bg-white border border-slate-200 rounded-xl focus:ring-rOrange" required>
                                <span class="text-slate-400 font-bold">-</span>
